...
@observer
export default class Home extends React.Component<{}, { open: boolean; services: number[] }> {
                ...
                <div className="row">
                    <div className="col">
                        <Tabs defaultActiveKey="orders" id="uncontrolled-tab-example">
                            ...
                            {appStore.isAuthorize && <Tab eventKey="place-order" title={<Link to="place-order">Place Order</Link>} />}
                            {appStore.isAuthorize && !appStore.isExecutor && (
                                <Tab eventKey="become-executor" title={<Link to="become-executor">Become Executor</Link>} />
                            )}
                        </Tabs>
                    </div>
                    <div
                    ...
                    >
                        {(servicesStore.services || []).map((serviceClassifer: any, index: number) => {
                                ...
                                >
                                    <Card.Header>
                                        <Button
                                            ...
                                            aria-controls={'service' + serviceClassifer.id}
                                    ...
                                    <Card.Body>
                                        {(services || []).map((service: any, index: number) => {
                                            ...
                                            return (
                                                <Collapse key={id + index} in={this.state.open}>
                                                    <div className="row">
                                                        <div className="col-lg-2">
                                                            <input
                                                                ...
                                                                id={'get-service' + id}
                                                                className="checkbox"
                                                                onChange={(event: any) => this.onSelectService(event, id)}
                                                            />
                                                        </div>
                                                        <div className="col">
    ...
    onSelectService(event: any, id: number) {
        ...
        ApiService.getData(ApiUrls.OrdersUrl, params).then((response: any) => {
            ordersStore.orders = response.orders;
            ordersStore.pagination = response.pagination;
        });

        ApiService.getData(ApiUrls.ExecutorsUrl, params).then((response: any) => {
